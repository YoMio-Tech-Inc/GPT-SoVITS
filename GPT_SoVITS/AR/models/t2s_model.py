...
class T2SBlock:
    ...
    def process_prompt(
        self, x, attn_mask: torch.Tensor, padding_mask: torch.Tensor = None
    ):
...
class Text2SemanticDecoder(nn.Module):
    ...
    def infer_panel_batch_infer_with_flash_attn(
        ...
        **kwargs,
        ...
        x = torch.stack(
            x_list, dim=0
        )  # ! TODO: x没有意义，因为这里x被提取了embedding，但是bert_feature已经是embedding了。
    ...
    def infer_panel_0307(
        ...
        **kwargs,
        ...
        for i in range(len(x)):
            y, idx = self.infer_panel_with_flash_attn_only(
                x[i].unsqueeze(0),
                x_lens[i],
