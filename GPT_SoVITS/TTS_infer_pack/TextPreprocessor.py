...
class TextPreprocessor:
    def __init__(
        self,
        bert_model: AutoModelForMaskedLM, # TODO: 如果是BGE M3，如何更改这个类型？
    ...
    def preprocess(self, text: str, lang: str, text_split_method: str) -> List[Dict]:
        ...
        for text in tqdm(texts):
            phones, bert_features, norm_text = (
                self.segment_and_extract_feature_for_text(text, lang)
            )
            if phones is None:
                continue
            res = {
    ...
    def get_bert_feature(self, text: str, word2ph: list) -> torch.Tensor:
        with torch.no_grad():
            ...
            res = self.bert_model(**inputs, output_hidden_states=True) # TODO: 模型换成BGE-m3
            res = torch.cat(res["hidden_states"][-3:-2], -1)[0].cpu()[1:-1]
        assert len(word2ph) == len(text)
        phone_level_feature = [] # TODO: 没有音素，不要这个
        for i in range(len(word2ph)): 
            repeat_feature = res[i].repeat(word2ph[i], 1)
            phone_level_feature.append(repeat_feature)
        phone_level_feature = torch.cat(phone_level_feature, dim=0)
        return phone_level_feature.T # TODO: 返回的也是没有按音素延长版本。
