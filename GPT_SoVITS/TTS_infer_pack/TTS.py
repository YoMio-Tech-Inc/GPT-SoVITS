...
class TTS:
    def __init__(self, configs: Union[dict, str, TTS_Config]):
        ...
        self.precision: torch.dtype = (
            torch.float16 if self.configs.is_half else torch.float32
        )
            ...
            for item in item_list:
                if prompt_data is not None:
                    ...
                    all_phones = torch.LongTensor(
                        prompt_data["phones"] + item["phones"]
                    ).to(device)
    ...
    def run(self, inputs: dict):
        ...
        if parallel_infer:
            ...
            self.t2s_model.model.infer_panel = (
                self.t2s_model.model.infer_panel_batch_infer_with_flash_attn
            )
        else:
        ...
        if not no_prompt_text:
            for i, prompt_text in enumerate(prompt_texts):
                ...
                if i == 0:
                    ...
                    self.prompt_cache["phones"] += phones
                    self.prompt_cache["bert_features"] = torch.cat(
                        [self.prompt_cache["bert_features"], bert_features], 1
                    )
                    self.prompt_cache["norm_text"] += norm_text
        ...
        try:
            ...
            for item in data:
                ...
                refer_audio_specs: list[torch.Tensor] = [
                    ...
                    for item in self.prompt_cache["refer_specs"]
                ]
                ...
                audio_frag_end_idx = [
                    sum(audio_frag_idx[: i + 1]) for i in range(0, len(audio_frag_idx))
                ]
                all_pred_semantic = (
                ...
                batch_audio_fragment = [
                    _batch_audio_fragment[
                        audio_frag_end_idx[i - 1] : audio_frag_end_idx[i]
                    ]
